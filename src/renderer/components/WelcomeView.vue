            ...
            <v-card>
                <v-card-text>
                    ...
                    <p>
                        ...
                        target="_blank">documentation</a>. If you have questions, please join the official <a
                        href="https://chat.vuetifyjs.com/"
                        target="_blank"
                        title="chat">discord</a>. Find a bug? Report it on the github <a
                        href="https://github.com/vuetifyjs/vuetify/issues"
                        target="_blank"
                        title="contribute">issue board</a>.
                    </p>
                    <p>
        ...
        <v-flex xs10 class="mt-4">
            <v-card>
                ...
                <v-card-actions class="pt-3 pb-3">
                    <v-spacer></v-spacer>
                    <v-btn class="link-btn" @click="test()">Test</v-btn>
                    <v-spacer></v-spacer>
    ...
    methods: {
      ...
      test() {
        if (!localStorage.catalog) {
          const res = this.$electron.remote.dialog.showOpenDialog({properties: ['openDirectory']});
          if (res && res[0]) {
            localStorage.catalog = res[0];
          } else {
            return;
          }
        }
